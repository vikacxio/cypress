...
var monthtimes = mon - monthnumber+2;//changed 1 to 2
...
class DebtorFooterSection{
aboutUs(){
    cy.wait(2000)
cy.xpath('//a[text()="About Us"]').click({force:true})
cy.wait(2000)
...
faqs(){
    ...
    cy.xpath('//span[@class="navbar-brand-logo"]').should('exist')
    cy.xpath('//span[@class="section-title ml-2"]')
    //.should('have.text', 'FAQs')
    .each(($el, index, $list) => {
        var name = $el.text()
        cy.log(name)
        expect($el).to.have.text('FAQs')
...
export default DebtorFooterSection