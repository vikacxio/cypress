      ...
      .each(($el, index, $list) => {
        ...
        if (debtrole !== "Others") {

          {
            ...
            cy.get('#email').click({ force: true }).type(fakeEmail)
        ...
        //Selecting only others debtor
         ...
         // cy.contains('Your Name')
          cy.contains("Entity's Name")
          cy.contains('Share a few more details about ')
          cy.contains('the entity')
          cy.contains(' to help us create a customised dashboard.')
        ...
        if (credrole !== "Secured Financial Creditor") {
...

          {
            ...
            cy.get('#btnSignIn').then($button => {
              if ($button.is(':disabled')) {
                //you get here only if button is disabled
                cy.xpath('//div[@class="errortxt error-txt break-txt mno-styles3"]')
                  .each(($el, index, $list) => {
                    var errmsg = $el.text()
                    cy.log(errmsg)
